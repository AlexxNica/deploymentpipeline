		...
		// BEGIN - added for send mail testing
 		...
 		if ( javamail != null && javamail.length() > 0 ) {
 			...
 			try {
 				...
 				props.put("mail.smtp.host", System.getProperty("emailhostname")); // eMail.setHostName
 		...
 		if ( springmail != null && springmail.length() > 0 ) {
 			...
 			try {
 				sender.setHost(System.getProperty("emailhostname")); // eMail.setHostName
 		...
 		if ( commonsmail != null && commonsmail.length() > 0 ) {
 			...
 			try {
 				email.setHostName(System.getProperty("emailhostname"));
