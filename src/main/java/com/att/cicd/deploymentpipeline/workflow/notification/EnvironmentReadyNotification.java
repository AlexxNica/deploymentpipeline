...
public class EnvironmentReadyNotification implements JavaDelegate {

	public static void main(String[] args) {
		// EnvironmentReadyNotification c = new EnvironmentReadyNotification();
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		if (!array_indexer.equals("")) {
			execution.setVariable("array_indexer", String.valueOf(Integer.valueOf(array_indexer) + 1));
		}
		ApplicationLogger.callLog(gotsid, name, pipelineId, pipelineName, submoduleName, processName,
				"EnvironmentReadyNotification", "execute", "Beginning");
		...
		ApplicationLogger.callLog(gotsid, name, pipelineId, pipelineName, submoduleName, processName,
				"EnvironmentReadyNotification", "execute", "End");
	}
	...
	private void mail(String recipient, String gotsid, String name, String processName, String submoduleName,
		...
		String sql = "Select (SELECT attribute_value FROM camundabpmajsc6.system_config where attribute_name='environmentReadyEmailBody') as emailBody, "
				+ "(SELECT attribute_value FROM camundabpmajsc6.system_config where attribute_name='emailFromAddress') as emailFromAddress, "
				+ "(SELECT attribute_value FROM camundabpmajsc6.system_config where attribute_name='emailUsername') as emailUsername, "
				+ "(SELECT attribute_value FROM camundabpmajsc6.system_config where attribute_name='emailPassword') as emailPassword, "
				+ "(SELECT attribute_value FROM camundabpmajsc6.system_config where attribute_name='emailSenderName') as emailSenderName, "
				+ "(SELECT attribute_value FROM camundabpmajsc6.system_config where attribute_name='emailPort') as emailPort, "
				+ "(SELECT attribute_value FROM camundabpmajsc6.system_config where attribute_name='clientURL') as clientURL, "
		...
		try {
			...
			while (rs.next()) {
				...
				emailFromAddress = rs.getString("emailFromAddress");
				emailUsername = rs.getString("emailUsername");
				emailPassword = rs.getString("emailPassword");
				emailSenderName = rs.getString("emailSenderName");
				emailPort = rs.getInt("emailPort");
				clientURL = rs.getString("clientURL");
